...
const reducer = (state, action) => {
  ...
  if (action.type == ADD_FILE) {
    if (!action.playload.name) {
      const files = [
        ...state.files,
        {id: action.playload.id, name: '', fileContent: ''},
      ];
      return {...state, files};
    } else {
  ...
  return (
    <li
      ...
      className={`files-list__item list-item ${
        file.selected || state.files.length === 1 ? 'selected' : ''
      }`}
    >
      {!file.id == 0 ? (
        <button className="btn btn__del" onClick={deleteFile}>
          <p>x</p>
        </button>
      ) : (
        ''
        ...
        <div className="files">
          ...
          <ul className="files__list files-list">
            {state.files.map(file => (
              <NewFile key={file.id} file={file} removeFile={removeFileItem} />
            ))}
          </ul>
          <button
    ...
    <main className="app-wrapper__main main">
      ...
      >
        ...
        <p
          ...
          //dangerouslySetInnerHTML={renderHTML(editedText)}
        ></p>
      </div>
    </main>
  );
...
const FileEidtor = ({selectedFile,  saveText}) => {
